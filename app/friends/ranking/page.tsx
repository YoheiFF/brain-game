...
import {
  GAME_IDS, GAME_META, type GameId,
  type RankEntry, type OverallEntry,
} from "@/lib/scores";
import { calcGamePoints } from "@/lib/game-points";
import { getNickname, getUserId } from "@/lib/nickname";
...
export default function FriendRankingPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      fetch(`/api/friends/ranking?userId=${userId}`).then((r) => r.json()),
      fetch(`/api/friends?userId=${userId}`).then((r) => r.json()),
    ])
      .then(([rankData, friendData]) => {
        setGameRankings(rankData.gameRankings ?? {});
        setOverall(rankData.overallRanking ?? []);
        setHasFriends(Array.isArray(friendData) && friendData.length > 0);
      })
      .catch(console.error)
  ...
  return (
    <main className="max-w-2xl mx-auto px-4 py-8 min-h-screen">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-3xl font-black text-white">👥 フレンドランキング</h1>
      </div>
...
function GameCard({
  ...
  e: RankEntry; myNick: string | null; unit: string; gameId: GameId;
}) {
...
function EmptyState() {
  return (
    <div className="card p-12 flex flex-col items-center gap-3 text-center">
      ...
      <p className="text-[#64748b] text-sm">フレンドがゲームをプレイするとランキングに表示されます</p>
    </div>
  );
}

function NoFriendsState() {
  return (
    <div className="card p-12 flex flex-col items-center gap-3 text-center">
      <p className="text-4xl">👥</p>
      <p className="text-white font-bold">フレンドを追加してから確認しよう</p>
      <p className="text-[#64748b] text-sm">フレンドと一緒にスコアを競おう！</p>
      <Link href="/friends" className="btn-primary mt-2">フレンドを追加する</Link>
    </div>
  );
}
